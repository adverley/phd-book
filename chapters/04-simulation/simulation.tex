...
\begin{equation}
    ...
    \vec{v}(t+\Delta t) &=\vec{v}(t)+\Delta t \cdot(\vec{f} / m)
    \end{aligned}
\end{equation}
...
\begin{equation} \label{eq:verlet}
    \vec{x}(t + \Delta t) = \alpha \vec{x}(t) + (1 - \alpha) \vec{x}(t - \Delta t)
\end{equation}
...
The springs introduced in \cref{subsec:sim_particle_system} function as constraints and flow of energy in the particle system. Hence we will use the terms spring and constraint interchangeably. The topology of these connections will determine the global behavior of the cloth. In \cref{fig:topology_cloth}, we create a rectangular grid with particles that are evenly spaced. We connect each particle with its second-order neighborhood which leads to three types of springs \autocite{Provot1995DeformationCI}. The first type of springs are the \emph{structural springs}, drawn in orange, that resist stretching the lattice.
The shear springs, indicated in green, connect diagonal neighbors and resist shearing forces. This prevents the cloth from collapsing inwards.
The last spring type, the bending spring drawn in purple, connect the second order horizontal and vertical neighbors. This type of spring allows to resist bending for local patches when the resolution is high.
...
\begin{itemize}
    ...
    \item To implement cloth-cloth collision, we utilize a brute-force approach in which we compare every particle on the mesh with every other particle. This exhaustive comparison is feasible because we implement the simulation on GPU that allows checking every particle in one pass. 
    \item Cloth-gripper contact modeling is simplified considerably by assuming the end-effector snaps the cloth once it is in close proximity. We utilize this magnetic-like approach to avoid complex contact modelling.
\end{itemize}

\subsection{Rendering}
Although we stated that a particle is a volumeless entity, we can choose how to represent the point mass. The idea is that if we know the location of a particle, we can place the desired graphical representation at that location. We visualize the cloth by placing vertices at the locations of the particles. Then, we triangulate by connecting east and south neighbors. We calculate the normal vector on this triangle for shading using Blinn-Phong shading \autocite{Blinn1977}. Texturing is done by sampling cloth texture images found on the internet. 

\subsection{Implementation results}
We implement the cloth simulation described in this section using Unity. In order to speed up calculations, we implement the simulation on GPU using shaders. We advance the cloth simulation using a fixed delta time of \qty{0.02}{\second} for all physics steps in order to have determinstic cloth physics. 

\cref{fig:cloth_sim_result} shows the resulting cloth. 
    rectangular tafelkleed laten zien.
    eventueel Een handdoek maken. 
    Jeans en broek voorbeeld.

Wat schrijven over de performance en resolutie, collision handling. 
...
\footnote{The implementation of the simulation is developed in context of a software engineering course with the following students: Niels Dossche, Glenn Feys, Jonas Bovyn, Tibo Vande Moortele, Stefan Croes, Julien Marbaix, and Laurens Debackere.}


\section{Conclusion }
considerations, approaches, our approach, DRL works but reward function godmode and many iterations
hier zeg ik: zet in conclusies ook limitaties + wat met de huidige evoluties -> cf vraag wat zou je nu kiezen als simulator
