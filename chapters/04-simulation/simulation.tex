...
\begin{itemize}
    \item a simulation tool able to simulate robots and the associated physics.
    \item simulate cloth.
    \item communicate with the physical robot.
    \item easily interfacing the simulation environment with off-the-shelf machine learning and reinforcement learning libraries that often require a Python interpreter. 
\end{itemize} 
...
Performing an in-depth, exhaustive comparison between popular robotics simulators for manipulation tasks is a difficult task. A thorough comparison would require setting up multiple scenarios, relevant metrics and experience with all the considered simulation technologies. For example, it is unclear whether we should compare accuracy, scalability, stability or speed. Furthermore, it is hard to concretely quantify such metrics. For example, accuracy is difficult to quantify in the absence of analytical solutions.
...
\begin{table}[htb]
    ...
    \begin{tabular}[t]{c c c c c}
        ...
        Inverse kinematics          & \checkmark         &                      &   \checkmark    &   $\pm$\tnote{\S} \\
...
\begin{figure}[htpb]{}
    ...
    \caption[]{Simulated Baxter: PyBullet vs Unity.}
...
\begin{figure}[htb]
    ...
    \caption[Mass-spring particle system.]{Mass-spring particle system: a rectangular grid of particles connected by different spring types.}
...
\begin{algorithm}[htpb]
    ...
    Initialize prioritized replay memory $\mathcal{H}=\emptyset$, $\Delta = 0$, $p_1=1$\;
    Initialize online action-value function $Q$ with random weights $\theta$\;
    Initialize target action-action function $\hat{Q}$ with weights $\theta^{\prime}$\;
    ...
    \ForEach{episode}{
        ...
        \ForEach{step of episode}{
            ...
            \For{ $j=1$ \KwTo $k$}{
                ...
                Compute TD-error $\delta_{j}=r_{j}+\gamma Q_{\theta^{\prime}}\left(s_{j}, \argmax_{a} Q\left(s_{j}, a\right)\right)-Q_{\theta}\left(s_{j-1}, a_{j-1}\right)$\;
...
The state space $\mathcal{S}$ consists of the joint angles of the Baxter robot and the positions of the cloth particles relative to the robot end-effectors. 
The action space $\mathcal{A}$ is in joint space: for every joint, the agent can move according to a fixed angle in any direction. 
...
\begin{figure}[p]{}
    ...
    \caption[Mean reward per episode for the subtasks during training.]{Mean reward per episode for the subtasks of learning how to fold a cloth twice in simulation.}
...
\begin{figure}[htbp]
    \centering
    % \includesvg[width=\textwidth]{figures/total_success_rate.svg}
    \includegraphics[width=\textwidth]{figures/total_success_rate}
    \caption[Success rate at test time for folding cloth twice in simulation.]{Success rate at test time for folding cloth twice in simulation. Evaluated on 100 episodes of the full folding sequence.
    }
    \label{fig:sim_results_success_rate_test}
\end{figure}

\section{Conclusion}
Simulation is an indispensible tool for the robotics researcher as it allows fast experimentation and massive, inexpensive data generation. The robotics simulator landscape is large and choosing a specific technology requires specifying and trading-off domain-specific requirements. For our application of cloth folding, we found the support for cloth simulation in existing robotics simulators to be lacking. Therefore, we decided to use Unity as robotics simulator and implement a custom cloth simulation on GPU using a mass-spring methodology. We found that a \gls{DQN} agent can learn to fold a rectangular piece of cloth twice in simulation within \qty{24}{\hour} of wall time on standard computational hardware. However, it is unclear whether such approach transfers to the real world. 
First, Sim2Real methods like domain randomization can be used for transfer but require a lot of compute.
Second, we require a lot of interactions in simulation. Learning from scratch on a real robot would take an exorbitant amount of time. Assuming a roll-out would consume one minute of robot time, this experiment would almost lasted half a year to complete. 
Third, it is difficult to obtain the cloth state which is used both as observation for the agent and reward function.
