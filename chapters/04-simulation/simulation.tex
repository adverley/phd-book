...
\begin{algorithm}[htpb]
    ...
    \KwIn{Discount factor $\gamma$, minibatch size $k$, target network update frequency $C$, prioritization importance $\alpha$ and importance sampling exponent $\beta$, learning rate $\eta$}
...
In our experiments, we represent the Q-function with a deep feedforward neural network containing X layers with 64 \texttimes 64 \texttimes 64 hidden neurons. 
...
We multiply the cost function $\operatorname{Cost}$ with minus one in order to arrive at the reward for the agent. 


\subsection{Results}
plots: 
    Success rate per episode during training \cref{fig:sim_results_success_rate_training}
    Mean reward per episode for the subtasks during training \cref{fig:sim_results_reward}
    success rate per task during test time \cref{fig:sim_results_success_rate_test}

\begin{figure}[htbp]
    \centering
    % \includegraphics[width=\textwidth]{}
    \caption[Success rate per episode during training.]{Success rate per episode during training.}
    \label{fig:sim_results_success_rate_training}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % \includegraphics[width=\textwidth]{}
    \caption[Success rate at test time.]{Success rate at test time.}
    \label{fig:sim_results_success_rate_test}
\end{figure}

om te tonen dat leren mogelijk is in deze setup maar wat de moeilijkheden zijn: de reward, godmode en aantal obs.
...
Korte samenvatting: learning in simulation 
Simulation works. Great tools. Nice that large game engines are jumping on the robotics train. Fast, safe and accessible experimentation. 
BUT will this work on a real robot?
First, Sim2Real methods exist for transfer but require a lot of compute!
Second, we still require a lot of interactions in simulation. Learning from scratch on a real robot would take months.
Third, where do we get the cloth state which is used both as observation for the agent and reward function?
