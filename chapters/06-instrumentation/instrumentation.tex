...
\setlaymanexpl{
Many robotic experiments stay in simulation. We bridge the gap between simulation and the real world by estimating the state of cloth using smart textile technology.
\\TODO!(of)\\
We train a robot to fold a smart cloth in-vivo. 
\\ (of trek het veel verder)\\
Outsource intelligence from the robot to the environment. 
We show that a robot can learn to fold clothing in-vivo by building a smart textile with integrated low-cost sensing. 
% This instrumented approach demonstrates the potential for accelerating learning by integrating vision, proprioception and tactile sensing on robot, object and environment.
}
