...
\setlaymanexpl{Future general-purpose robots are multi-modal systems leveraging preprogrammed priors and data with embodied intelligence to understand and act on the world.}
% the future is multi-modal, residual and embodied intelligence
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=\linewidth, keepaspectratio]{figures/datasets_3_defaced.jpg}
    \caption[An example for crowdsourcing data for robotics research.]{Our folding table setup to crowdsource data. A citizen science approach to crowdsourcing data can minimize researcher's bias in datasets. The folding table provides structure in the observations between demonstators.}
...
\begin{figure}[htpb]
    ...
    \begin{subfigure}[b]{width=0.45\textwidth}
        \includegraphics[width=\linewidth,keepaspectratio]{figures/sim_constraints.png}
        \caption{Source: \autocite{Matas2018}}
        \label{fig:towards_simulation_constraints}
    \end{subfigure}

    % \begin{subfigure}[b]{width=0.45\textwidth}
    %     \includegraphics[width=\linewidth,keepaspectratio]{figures/sim_contacts.png}
...
% a large role will be reserved for accessible hardware and the size of the robot's skill repertoire. Acquiring dexterous manipulation skills requires dexterous robotic hands that are able to sense the environment, build a representation of how the world works and use that feedback to understand how its action influences the environment, such as deformations on deformable bodies. Co-optimization to achieve a holistic system where hardware closes the loop with control.
