...
% YOU ARE HERE!!!
% 	Herlees en pas aan
% 	Literatuur stukje van TCN is verwerkt maar het kan meer structuur gebruiken. zie TODO 
...
\note{Check: is het duidelijk waarom leidt dit tot nuttige embedding?}

\note{De concrete aanpak pas bespreken in TCN hoofdstuk!}

\textit{enkele relevante CSSL werken in cloth domain bespreken}
% TODO: dit kan meer structuur gebruiken
...
% \textit{oorspronkelijke tekst}
% In~\cite{Kompella2017}, this is done by employing clustered slow feature analysis signals as a representation of the environment. A curiosity-based reward encourages an agent to focus on learning ill-understood abstractions. However, the authors state that it is unclear to what extent their method learns invariances, given that the learned abstractions operate on low-level details. Furthermore, their method does not extract nonlinearities in the input images as they employ a linear slow feature analysis method. This greatly limits the complexity of the abstractions that can be encoded. In contrast, we conduct an in-depth analysis to show that our network encodes representations containing useful high-level invariances using non-linear estimators. 
 
% Additionally, our method does not employ curiosity as it might force the agent to attend to learn attributes irrelevant to solving the task. Another approach is investigated in~\cite{Ramirez2017}, in which an activity recognition system is built by extracting semantic rules from segmented hand trajectories and object properties. They use this inferred target activity as input for a planning system. However, this approach relies heavily on accurate hand detection using an engineered vision system. Our approach to encoding semantics from the scene is trained end-to-end, which allows the system to deduce the relevant features for encoding the task instead of using only inputs that were engineered by hand.

% An emerging method for sample-efficient learning of task-relevant features is self-supervised learning. Self-supervised learning exploits the structure present in a dataset to learn rich representations which can be used for a downstream task such as image classification.
% Both natural language processing and computer vision has seen large leaps in self-supervised methods with for example BERT~\cite{Devlin2018}. The general idea is to provide an artificial task to learn meaningful representations. Example tasks are learning to colorize images~\cite{Zhang2016Color}, reconstructing the original input~\cite{Pathak2016} and predicting the relative position of two random patches~\cite{Doersch2015}. An instance of self-supervised learning is contrastive learning, in which representations are learned by providing contrasting examples. In the case of video demonstrations, time can be used as a supervisory signal to provide contrasting examples. The goal then becomes to recover the temporal coherence of a video. One of the firsts works leveraging time as contrastive signal~\cite{Misra2016} inputs a sequence of frames and classifies whether the frames are in the correct order. Later work~\cite{Lee2017,Fernando2017} also frames self-supervised learning as a classification task in which the correct temporal order has to be determined.

% A number of prior works construct reward functions, or equivalently process monitoring metrics, in latent spaces trained with time as a supervisory signal.
% In~\cite{Singh2019}, they construct a reward function based on an image classifier trained on successful goal states reached by teleoperating the robot towards the end state. In~\cite{Hartikainen2019}, time is used as a learned distance function for assigning environment rewards. However, their approach requires human intervention in order to select the desired goal states. ~\cite{Nair2018time} also uses time as a supervisory signal in videos of expert demonstrations to learn an optimal trajectory of states. However, they assume the possibility of visually removing the end-effector from the scene which is not possible for all tasks. % (bv je kan niet in het midden van toevouwen je stuk kledij laten vallen).
% Other work~\cite{Nair2018visual} looks at expressing the reward function as the distance in latent space between the current state and the goal state. However, this is not possible when there is a trajectory in latent space that has to be followed in order to execute the task. % bv. bij het cube stacking blok gaat bij de eerste stap (grijp een kubus) de embedding waarschijnlijk altijd op dezelfde plaats liggen als op het einde vd taak dus je moet hier tussen kunnen differnetieren.  
% %Alternatively, it is possible to add a contrastive loss as an auxiliary objective in RL as done in CURL~\cite{Srinivas2020CURL}. The authors show that their method outperforms other learning methods on the DM Control Suite.
