...
% TODO IDEE: er moet een blauwe rechthoek op de kaft. Kunnen we er dit laten uitzien als een blauw cloth? Zie de twee fotos in ideas folders. 
...
% Choose a math font
% When using the unicode-math package along with a suitable OpenType math font (specified via a suitable \setmathfont instruction), there's no need to load the following packages: lmodern, fontspec, amsmath, amsfonts, amssymb, and bm.
...
% \usepackage{mathtools} % for auto braces command next in mathcmds.tex but this breaks underbrace command so avoid
\usepackage{textcase}
...
% BIBLATEX: fullcite: do not use et al!
\makeatletter
...
\newtcolorbox{keyMessageBox}{textmarker,
    borderline west={6pt}{0pt}{ColorMainDark},
    colback=ColorMain,
    colbacktitle=ColorMainDark,
}
\newtcolorbox{keyMessageBoxWithTitle}[1]{
    textmarker,
    borderline west={6pt}{0pt}{ColorMainDark},
    colback=ColorMain,
    title=#1,
...
\newtcolorbox{keyMessageBoxInText}{
    textmarker,
    borderline west={6pt}{0pt}{ColorMainDark},
    colback=green!30!blue!5!white,
    colbacktitle=ColorMainDark,
    boxrule=0mm,
    colframe=white
}
\newtcolorbox{keyMessageBoxInTextWithoutBorder}{
    textmarkerNoWest,
    colback=green!30!blue!5!white,
...
\newcommand{\keyInText}[1]{\begin{keyMessageBoxInText} #1 \end{keyMessageBoxInText}}
\newcommand{\keyInTextWithoutAccent}[1]{\begin{keyMessageBoxInTextWithoutBorder} #1 \end{keyMessageBoxInTextWithoutBorder}}
...
% \usepackage{colortbl} % for row and col colors in tables
...
\usepackage{xurl} % decent linebreaks in \url command 

% Nicier math breaklines --> these dont work with the new math font package we use 
% \usepackage{flexisym}
% \usepackage{breqn}

\input{config/styles.tex}

\title{Learning Robotic Cloth Manipulation} 

\def\thetitleNL{Methodes voor het leren manipuleren van kledij met robots}
...
% ---- SCHUDBLADEN TUSSEN HOOFDSTUKKEN ----
% defines colour of pages between chapters
\newcommand{\chapterbookmarkcolor}{ColorMainDark}
\newcommand{\chapterbookmarktextcolor}{white}
\newcommand{\chapterbookmarklinecolor}{white}


% code needed to store the name of the current chapter
\usepackage{nameref}
\newcounter{secautolabel}
\setcounter{secautolabel}{0}
\AddtoDoHook{heading/endgroup/chapter}{%
	\stepcounter{secautolabel}%
	\label{sec:autolabel:\thesecautolabel}%
}

% store layman explanation of a chapter
\makeatletter
\def\chapter@laymanexpl{}

% set layman explanation
\newcommand{\setlaymanexpl}[1]{\gdef\chapter@laymanexpl{#1}}
\makeatother

% generates two additional blank coloured pages
\makeatletter
\AddtoDoHook{heading/begingroup/chapter}{%
	\if@mainmatter
	\pagecolor{\chapterbookmarkcolor}\thispagestyle{empty}
	\addtocounter{secautolabel}{1}%
	  {\begin{tikzpicture}[remember picture,overlay]
        \draw (current page.south east) ++(0,0) node[anchor=south east,inner sep=1cm, outer sep=1cm] {%
          \usekomafont{disposition}\usekomafont{chapter}\color{\chapterbookmarktextcolor}%
                {\fontsize{0.4\textheight}{0.6\textheight}\selectfont\ref*{sec:autolabel:\thesecautolabel}}% schrijf het hoofdstuknr
                % {\fontsize{0.3\textheight}{0.6\textheight}\selectfont .}% schrijf het puntje
            };
    \end{tikzpicture}}
	\addtocounter{secautolabel}{-1}% ----------------------
	\newpage\thispagestyle{empty}% SECOND PAGE
	\pagecolor{\chapterbookmarkcolor}\thispagestyle{empty}
	\addtocounter{secautolabel}{1}% START CONTENT SECOND CHAPTER PAGE
  \begin{tikzpicture}[remember picture,overlay]
    \draw (current page.center) ++(0,0) 
        node[anchor=center,inner sep=0pt, outer sep=0pt,text width=\textwidth,align=left] (chapterName) {%
          \usekomafont{disposition}\usekomafont{chapter}%
          {\color{\chapterbookmarktextcolor}\selectfont\nameref*{sec:autolabel:\thesecautolabel}}% schrijf het hfstnaam
      };
    \filldraw[fill=\chapterbookmarklinecolor,draw=\chapterbookmarklinecolor, line width=0.1cm] (chapterName.north west) ++ (0, 0.5cm)  -- ([yshift=0.5cm]chapterName.north east);
    \filldraw[fill=\chapterbookmarklinecolor,draw=\chapterbookmarklinecolor, line width=0.1cm] (chapterName.south west) ++ (0, -0.5cm) -- ([yshift=-0.5cm]chapterName.south east);
    \draw (current page.center) ++(0,0) 
        node[anchor=center,below = of chapterName,xshift=-0.25\textwidth,yshift=-0.5cm,inner sep=0pt, outer sep=0pt,minimum width=\textwidth,text width=0.5\textwidth,align=left] (chapterDescription) {%
          \usekomafont{disposition}\usekomafont{paragraph}% schrijf het hfst description
          {\color{\chapterbookmarktextcolor}\fontseries{m}\selectfont
          \makeatletter
          \chapter@laymanexpl
          \setlaymanexpl{}
          \makeatother
      ...
      \node[anchor=south west,yshift=2cm,inner sep=0pt, outer sep=0pt] at (chapterName.north west) (chapterNr) {%
            \usekomafont{disposition}\usekomafont{chapter}\color{\chapterbookmarktextcolor}%
            {\fontsize{0.4\textheight}{0.6\textheight}\selectfont\ref*{sec:autolabel:\thesecautolabel}}% schrijf het hoofdstuknr
            % {\fontsize{0.3\textheight}{0.6\textheight}\selectfont .}% schrijf het puntje
        };
  \end{tikzpicture}
	\addtocounter{secautolabel}{-1} 
	~\newpage\nopagecolor\thispagestyle{\chapterpagestyle}
	\fi
}
\makeatother
% ----
...
\usepackage{needspace} % package for better use of white space 
\usepackage{xpatch} % package for better use of white space from https://tex.stackexchange.com/questions/395666/how-to-force-section-to-go-to-new-page
\xpretocmd{\section}{\needspace{6\baselineskip}}{}{} % force section to new page if not enough space
...
\subfile{appendices/cloth_sim/cloth_sim.tex}
